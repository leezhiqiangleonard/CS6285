...
class Algo4(Adafactor):
	...
	def __init__(self,
		...
		super(Algo4, self).__init__(params,
			lr=lr,
			eps=eps,
	...
	def step(self, closure=None):
		...
		for group in self.param_groups:
			for p in group["params"]:
				...
				# State Initialization: Bias Corrected Initialization
				if len(state) == 0:
					state["step"] = 0
					...
					if factored:
						state["exp_avg_sq_row"].add_(grad.mean(dim=-1))
						state["exp_avg_sq_col"].add_(grad.mean(dim=-2))	
					else:
						state["exp_avg_sq"].copy_(grad)
					state["second_moment"].copy_(grad.norm().pow(2))
				...
				state["second_moment"].mul_(beta2).add_(norm, alpha=1.0 - beta2)
